fetch('../catalogs.json')
    ...
    .then(catalogs => {
        window.catalogs = catalogs; // Make catalogs globally available
        console.log('Catalogs fetched successfully:', window.catalogs);
    ...
    .catch(error => {
        ...
        const categoriesContainer = document.getElementById('categories-container');
        console.log('categoriesContainer in catch block:', categoriesContainer);
        if (categoriesContainer) {
            categoriesContainer.innerHTML = '<p>Error loading catalogs.</p>';
        }
    })
    .finally(() => {
        console.log('main.js: All catalogs processed. Dispatching "catalogsLoaded" event.');
        window.dispatchEvent(new Event('catalogsLoaded'));
    });